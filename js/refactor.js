...
window.onload = function(){
    class TranslateToMorse{
        constructor(){
            ...
            this.output = document.getElementById('translated');
        ...
        translate(){
            ...
            const inputToCode = inputToArray.map(letter => {
                if (!alphaToCode[letter]){
                    return [];
                }
                return alphaToCode[letter]
            });
            const morseLetter = inputToCode.flatMap(letter => {
                const symbols = letter.map(symbol => this.morse(symbol));
                if(symbols.length !== 0){
                    symbols.push(this.space());
                }
                return symbols;
            });
    ...
    const morseCode = new TranslateToMorse();
}